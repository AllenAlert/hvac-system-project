...
A Python-based HVAC engineering and simulation platform.

**Author:** Bola

## About

This project provides HVAC engineering calculations, real-time simulation,
and machine learning predictions through a web dashboard.

The core engineering formulas are adapted from open-source references and
standard ASHRAE/EN methodologies. The dashboard, simulation engine, and ML
integration were developed by me.

**Features:**
- Heating/cooling load calculations (ASHRAE RTS method, EN 12831)
- Energy consumption estimation (bin method)
- Real-time HVAC simulation with multiple control strategies
- ML-based energy and load prediction
...
Some engineering calculations reference formulas from:
- ASHRAE Handbook of Fundamentals
- EN 12831 (Heating load calculation)
- Various open-source HVAC libraries
...
A **web dashboard** is included to run HVAC calculations in the browser:

- **Heating load** — Building heating load, balance temperature, and load curve (Q̇ vs outdoor temp).
- **Solar** — Solar position, sunrise/sunset, and surface angles for any location and date.
- **Radiator** — Panel radiator design from nominal specs and operating point (flow, supply temp, or heat output).

### Run the dashboard

**Option 1 — Conda (recommended, uses `environment.yml`)**

1. Create and activate the environment (once):

   ```bash
   conda env create -f environment.yml
   conda activate python-hvac
   ```

   If the env already exists but is missing dashboard deps, update it:
...
2. From the project root, start the dashboard:

   ```bash
   uvicorn dashboard.main:app --host 127.0.0.1 --port 8000
   ```

   Or on Windows, double‑click **`run_dashboard.bat`** (it activates `python-hvac` and runs uvicorn).
...
**Option 3 — pip (after installing the project, e.g. `pip install -e .`)**

```bash
pip install "fastapi[standard]" "uvicorn[standard]" jinja2 python-multipart
uvicorn dashboard.main:app --reload
```
